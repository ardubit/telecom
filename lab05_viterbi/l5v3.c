...
// char coded[2 * MAXSIZE] = {0, 0, 1, 1, 1, 0, 0, 0, 0, 1, 1, 0, 0, 1, 1, 1, 0, 0};
...
char getMetricsHemm(char *K, int i0, int i, int j) {
  char Hd = 0;
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (j < m) {
        ...
        if (((i == 0) || (i == 1)) && (j != 0)) {
          ...
          ways[i][j] = i;
          Hd[i][j] = Hd[0][j - 1] + getMetricsHemm(K, 0, i, j);
        }
        ...
        if (((i == 2) || (i == 3)) && (j != 0)) {
          ...
          ways[i][j] = i - 2;
          Hd[i][j] = Hd[1][j - 1] + getMetricsHemm(K, 1, i, j);
        }
        ...
        if ((i == 0) || (i == 1)) {
          copy = 0;
          Hd[i][j] = Hd[0][j - 1] + getMetricsHemm(K, 0, i, j);
          copy = Hd[2][j - 1] + getMetricsHemm(K, 2, i, j);
          if (copy < Hd[i][j]) {
            Hd[i][j] = copy;
            for (int k = 0; k < j; k++) {
        ...
        if ((i == 2) || (i == 3)) {
          copy = 0;
          Hd[i][j] = Hd[1][j - 1] + getMetricsHemm(K, 1, i, j);
          copy = Hd[3][j - 1] + getMetricsHemm(K, 3, i, j);
          if (copy < Hd[i][j]) {
            Hd[i][j] = copy;
            for (int k = 0; k < j; k++) {
...
int main(void) {
  char K[SIZE_DATA] = {0, 1, 0, 1, 1, 1, 0, 0, 0};
  // char K[MAXSIZE];
  int cnt = 0, cnt_BER = 0;
  int j = 2, n = 1;
